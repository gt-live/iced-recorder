...
impl CompositeStream {
    pub(crate) fn new_input<D, E>(
        stream_inner_a: StreamInner,
        stream_inner_b: StreamInner,
        mut data_callback: D,
        mut error_callback: E,
    ) -> CompositeStream
    where
        //D: FnMut(&Data, &InputCallbackInfo) + Send + 'static,
        D: FnMut(&Data, &Data, &InputCallbackInfo) + Send + 'static,
        E: FnMut(StreamError) + Send + 'static,
    {
        let pending_scheduled_event = unsafe {
            Threading::CreateEventA(None, false, false, windows::core::PCSTR(ptr::null()))
        ...
        let thread = thread::Builder::new()
            .name("cpal_wasapi_in".to_owned())
            .spawn(move || run_input_composite(run_context, &mut data_callback, &mut error_callback))
            .unwrap();
...
fn run_input_composite(
    mut run_ctxt: RunContext,
    data_callback: &mut dyn FnMut(&Data, &Data, &InputCallbackInfo),
    error_callback: &mut dyn FnMut(StreamError),
) {
    boost_current_thread_priority();
