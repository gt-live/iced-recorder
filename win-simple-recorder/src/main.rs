    ...
    let input_stream = match input_config.sample_format() {
        ...
        cpal::SampleFormat::F32 => {
            input_device.build_input_stream(
                &input_config.into(),
                //move |data, info: &_| {write_input_data::<f32, f32>(data, &writer_2)},
...
fn send_frame<T>(input: &[T], chan: Sender<Command<T>>)
where
    T: Sample,
    //U: Sample + hound::Sample + FromSample<T>, // + std::fmt::Display,
{
  ...
  //    return;
  //}
  chan.send(Command::Frame(input.to_vec())).unwrap();
  //chan.send(
  //  Command::Frame(
...
fn write_frame<T>(ra: Receiver<Command<T>>, rb: Receiver<Command<T>>, writer: &WavWriterHandle) -> Result<bool, anyhow::Error>
where
    T: Sample<Signed = T> + hound::Sample + std::fmt::Display,
{
    if let Ok(mut guard) = writer.try_lock() {
        if let Some(writer) = guard.as_mut() {
            ...
            loop {
                ...
                //println!("sample_a={}", sample_a.len());
                //println!("sample_b={}", sample_b.len());
                // TODO some way to customize it
                let mut a_iter = sample_a.into_iter();
                let mut b_iter = sample_b.into_iter();
                //let mut a_iter = sample_a.iter();
                //let mut b_iter = sample_b.iter();
                loop {
                ...
                //let (b0, b1) = match rb.recv() {
                //    Ok(Command::Frame(b0, b1)) => (b0, b1),
                //    _ => return Ok(false),
                //};
