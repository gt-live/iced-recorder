...
where
    ...
    //U: Sample + hound::Sample + std::fmt::Display + std::ops::Add,
{
    if let Ok(mut guard) = writer.try_lock() {
        if let Some(writer) = guard.as_mut() {
            //writer.write_sample(sample).ok();
            loop {
                let (a0, a1) = match ra.recv() {
                    Ok(Command::Frame(a0, a1)) => (a0, a1),
                    Err(e) => {
                ...
                writer.write_sample(a0).ok();
                writer.write_sample(a1).ok();
